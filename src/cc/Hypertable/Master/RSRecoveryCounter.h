    ...
    void result_callback(uint32_t attempt, std::vector<Result> &results) {
      ...
      foreach(Result &rr, results) {
        if (rr.error != Error::OK) {
          m_errors = true;
          HT_WARN_OUT << "Received error " << rr.error << " for range " << rr.range
              << HT_END;
        }

        std::set<QualifiedRangeSpec>::iterator set_it = m_outstanding_ranges.find(rr.range);
    ...
    bool wait_for_completion(Timer &timer) {
      ...
      while (m_outstanding_ranges.size()) {
        ...
        if (!m_cond.timed_wait(lock, expire_time)) {
          HT_WARN_OUT << "RSRecoveryCounter timed out" << HT_END;
          m_errors = true;
          m_timed_out = true;
